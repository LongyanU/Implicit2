...
% Oct 13 again   Elapsed time is 1700.004038 seconds.

%  325.726756 。April 23 2018.
clear
clc %%%%%%%
close all
...
load ('coeffJune17TraScheme.mat')
coeff=zeros(nz,nx,5);
for ii=1:nz
    for jj=1:nx
        
        coeff(ii,jj,:)=coeffJune(floor((v(ii,jj)-1486))+1,:);
...
for it=1:nt-2,
    ...
    for j=1:nx,
        d2pz1(:,j)=A1{j}\d2pz(:,j);   %%解三对角阵，直接matlab解了
    end
    for k=1:nz,
        d2px1(k,:)=A2{k}\d2px(k,:)';       %%解三对角阵，直接matlab解了
    end
